...
- **Octanos diferidos por moderación IA** (change `ai-comment-moderation`): desde la moderación por IA, los Octanos de posición se acreditan **en el momento en que el comentario pasa a `approved`**, evaluando la escalera solo entre comentarios `approved`. Un comentario en `pending_review` (dudoso o no validable) **no acredita ni consume puesto** hasta que un administrador lo aprueba; al aprobarlo se evalúa la escalera en ese instante. Un comentario `rejected` nunca acredita. Sin clawback por el borrado del **propio autor**: los Octanos ya acreditados no se revierten.
- **Retirada de Octanos por borrado admin** (change `admin-comments-management`): cuando un **administrador elimina** un comentario desde el panel (hard delete), **sí se retiran** los Octanos que dio ese comentario: se borran sus `octano_events` (liberando además el puesto de escalera) y se recalcula el total del autor. Es una acción de moderación (retirar contenido), distinta del soft-delete del autor.
